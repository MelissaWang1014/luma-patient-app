...
export async function linkSyntheticPatient(
  ...
  relationship: "caregiver" | "guardian" = "caregiver",
) {
...
export async function claimPatientInvitation(patientName: string, invitationCode: string) {
  const client = configured();
  const { data, error } = await client.rpc("claim_patient_invitation", {
...
export async function listMyPatients() {
  ...
  const { data, error } = await client
    .from("patient_access")
    .select("relationship,can_record,can_assign,patients(*)")
    .eq("status", "active");
  if (error) throw error;
  return data;
}

export async function recordCareObservation(input: {
  patientId: string;
...
export async function recordMedicationEvent(input: {
  ...
  status: "taken" | "missed" | "declined" | "unable" | "unknown";
  scheduledAt?: string;
  note?: string;
}) {
  const client = configured();
  ...
  const { data, error } = await client
    ...
    .insert({
      patient_id: input.patientId,
      medication_plan_id: input.medicationPlanId,
      recorded_by: user.id,
      status: input.status,
