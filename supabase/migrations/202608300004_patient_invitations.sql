...
begin
  ...
  if clinician_role not in ('clinician', 'admin') then
    raise exception 'Clinician access required';
  end if;
  ...
  plain_code := upper(substr(encode(gen_random_bytes(6),'hex'),1,8));
  insert into public.patient_invitations(patient_id,code_hash,created_by,expires_at)
  values(new_patient_id,crypt(plain_code,gen_salt('bf')),auth.uid(),expiry);
...
begin
  ...
  where lower(trim(p.display_name))=lower(trim(patient_name))
    ...
    and crypt(upper(trim(invitation_code)),i.code_hash)=i.code_hash
...
revoke all on function public.create_patient_invitation(text,date,text,text) from public;
revoke all on function public.claim_patient_invitation(text,text) from public;
grant execute on function public.create_patient_invitation(text,date,text,text) to authenticated;
grant execute on function public.claim_patient_invitation(text,text) to authenticated;
